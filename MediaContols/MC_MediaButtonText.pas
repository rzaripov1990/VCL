...
type
  ...
  TMediaButtonMode = (mbmNormal, mbmChecked, mbmCheckedGliph);
  ...
  private
    ...
    procedure SetGliphColor(const Value: TColor);
  ...
  published
    ...
    property GliphColor: TColor read fGliphColor write SetGliphColor default clBlack;
...
begin
  ...
  begin
    ...
    case fAlignment of
      taLeftJustify:
        begin
          if (fMode = mbmCheckedGliph) and (fChecked) then
          begin
            Brush.Style := bsClear;
            Pen.Style := psSolid;
            Pen.Color := fGliphColor;
            h := TextHeight('Hg');
            rc := Rect(0, 0, h, h);
            Rectangle(rc.Right div 4, (rc.Bottom div 4), rc.Right - (rc.Right div 4), rc.Bottom - (rc.Bottom div 4));
          end;

          TextOut(h + 1, (Height - TextHeight(Caption)) div 2, Caption);
        end;
      taRightJustify:
  ...
  if Transparent then
  begin
    fBuffer.TransparentColor := fTransparentColor;
...
begin
  ...
  begin
    ...
    begin
      ...
      if fMode = mbmCheckedGliph then
        Width := TextWidth(Caption) + TextHeight(Caption)
      else
...
procedure TZMSMediaButtonText.SetGliphColor(const Value: TColor);
begin
  fGliphColor := Value;
  Repaint;
end;
...
procedure TZMSMediaButtonText.SetTransparentColor(const Value: TColor);
begin
