...
uses
  WinApi.Windows, System.SysUtils, Vcl.Graphics, Types, QBCommon, SLCanvas32, Bass;
...
begin
  ...
  begin
    ...
    ABitmap.Canvas.CopyRect(Rect(0, 0, ABitmap.Width, ABitmap.Height), ABitmap.Canvas,
      Rect(BufferHScale, BufferVScale, ABitmap.Width - BufferHScale, ABitmap.Height - BufferVScale));
  end
end;
...
begin
  ...
  try
    ...
    if (@Soniq_QueryModule = nil) then
      @Soniq_QueryModule := GetProcAddress(DllHandle, '?QueryModule@@YAPAUUltraVisInfo@@XZ');
    ...
    begin
      ...
      begin
        ...
        Effects := ((SoniqVisInfo^.lRequired and SONIQUEVISPROC) = SONIQUEVISPROC);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Sample := MulDiv(128, ConvertSamples_32BitTo16(WaveData32[I * 2]), 32768);
        SoniqVisData.WaveForm[0, I] := Sample;

        Sample := MulDiv(128, ConvertSamples_32BitTo16(WaveData32[I * 2 + 1]), 32768);
        SoniqVisData.WaveForm[1, I] := Sample;
      end;
    end;
  except
  ...
  try
    ...
    begin
      ...
      begin
        ...
        SoniqVisData.Spectrum[1, I] := Min(255, Trunc(FFTData[I * 2 + 1] * 1024));
      end;
    end;
  except
    QBass_VisUnload;
...
const
  ...
  GetDataFFT: array [Boolean] of DWORD = (BASS_DATA_FFT2048 or BASS_DATA_FFT_INDIVIDUAL, BASS_DATA_FFT2048);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if BASS_ChannelGetData(Channel, @fft, GetDataFFT[chan.chans >= 2]) > 0 then
        ...
        begin
          if BASS_ChannelGetData(Channel, @w32, GetDataWave[chan.chans >= 2]) > 0 then
        ...
        begin
          if BASS_ChannelGetData(Channel, @w8, GetDataWave[chan.chans >= 2]) > 0 then
        ...
        begin
          if BASS_ChannelGetData(Channel, @w16, GetDataWave[chan.chans >= 2]) > 0 then
    ...
    // SoniqVisData.InputSource := 0;
    SoniqVisData.MillSec := BASS_ChannelSeconds2Bytes(Channel, BASS_ChannelGetPosition(Channel, BASS_POS_BYTE)) * 1000;

    if not SoniqVisInfo^.Render(VisBuffer.ScanLine[0], VisBuffer.Width, VisBuffer.Height, -VisBuffer.Width,
      @SoniqVisData) then
    begin
      QBass_VisUnload;
      asm
      ...
      BitBlt(DC, 0, 0, VisBuffer.Width, VisBuffer.Height, VisBuffer.Canvas.Handle, 0, 0, SRCCOPY);
...
var
  ...
  begin
    ...
    if (@temp_QueryModule = nil) then
      @temp_QueryModule := GetProcAddress(dll, '?QueryModule@@YAPAUUltraVisInfo@@XZ');
    ...
    begin
      ...
      begin
        ...
        Titles[Count] := tempVisInfo^.PluginName;
        Inc(Count);
