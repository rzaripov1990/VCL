...
type
  ...
  TWinampDSPModule = record
    ...
    ModifySamples: function(PDSPModule: PWinampDSPModule; Samples: Pointer; NumSamples: integer; BPS: integer;
      NCh: integer; SRate: integer): integer; cdecl;
...
begin
  ...
  @DSPGetHeader := GetProcAddress(DSPDLLHandle, PChar('winampDSPGetHeader2'));
...
procedure DSP32Proc(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; user: Pointer); stdcall;
var
...
begin
  try
    if (PluginLoaded) and (Handle > 0) and (Channel > 0) and (Assigned(DSPModule)) and (NumChan <= 2) then
    begin
      ...
      SamplesReturned := 0;
      try
        SamplesReturned := DSPModule^.ModifySamples(DSPModule, WinampSamplesForDSP, NumberOfBufferSamples, 16, NumChan,
          SampleRate);
      except
      end;
...
procedure DSP16Proc(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; user: Pointer); stdcall;
var
...
begin
  ...
  begin
    try
      ...
      if DSPModule^.ModifySamples(DSPModule, WinampBuf, NumberOfBufferSamples, 16, NumChan, SampleRate)
        = NumberOfBufferSamples then
...
begin
  if not InitDLL then
    Exit;
  Result := PAnsiChar(format('%s (%d)', [DSPHeader.Description, DSPHeader.Version]));
end;
// ******************************************************************************
