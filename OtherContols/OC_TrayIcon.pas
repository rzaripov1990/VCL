...
uses
  ...
  ExtCtrls, WaveTimer {$IFDEF DELPHI_4_UP},
  ImgList{$ENDIF}, ShellApi;
...
type
  TTimeoutOrVersion = record
    case Integer of // 0: Before Win2000; 1: Win2000 and up
      0:
        (uTimeout: UINT);
      1:
        (uVersion: UINT); // Only used when sending a NIM_SETVERSION message
  end;
  ...
  TNotifyIconDataEx = record
    ...
    hIcon: hIcon;
    szTip: array [0 .. 127] of Char; // Previously 64 chars, now 128
    dwState: DWORD;
    dwStateMask: DWORD;
...
{$ENDIF}
  ...
  TStartupEvent = procedure(Sender: TObject; var ShowMainForm: Boolean)
    of object;

  TZMSTrayIcon = class(TComponent)
  private
    FEnabled: Boolean;
    FIcon: TIcon;
    FIconID: Cardinal;
    ...
    FOnBalloonHintShow, FOnBalloonHintHide, FOnBalloonHintTimeout,
      FOnBalloonHintClick: TNotifyEvent;
    FMinimizeToTray: Boolean;
    FClickStart: Boolean;
    FClickReady: Boolean;
    CycleTimer: TMultimediaTimer; // For icon cycling
    ClickTimer: TMultimediaTimer; // For distinguishing click and dbl.click
    ExitTimer: TMultimediaTimer; // For OnMouseExit event
...
{$ENDIF}
    ...
    property MinimizeToTray: Boolean read FMinimizeToTray write FMinimizeToTray
      default False;
    // Minimize main form to tray when minimizing?
    // ----- END SPECIAL -----
    // Events:
...
procedure Register;
begin
  RegisterComponents('ZMSystem', [TZMSTrayIcon]);
end;
...
{ The message always goes through the container, TrayIconHandler.
  Msg.wParam contains the ID of the TZMSTrayIcon instance, which we stored
  as the object pointer Self in the TZMSTrayIcon constructor. We therefore
  cast wParam to a TZMSTrayIcon instance. }
...
begin
  ...
  begin
{$WARNINGS OFF}
    with TZMSTrayIcon(Msg.wParam) do // Cast to a TZMSTrayIcon instance
{$WARNINGS ON}
    begin
      case Msg.lParam of
...
procedure TZMSTrayIcon.ClickTimerProc(Sender: TObject);
begin
...
procedure TZMSTrayIcon.CycleTimerProc(Sender: TObject);
begin
...
procedure TZMSTrayIcon.MouseExitTimerProc(Sender: TObject);
var
  Pt: TPoint;
begin
  if FDidExit then
    Exit;
  GetCursorPos(Pt);
  if (Pt.X < LastMoveX) or (Pt.Y < LastMoveY) or (Pt.X > LastMoveX) or
    (Pt.Y > LastMoveY) then
  begin
    FDidExit := True;
    MouseExit;
...
constructor TZMSTrayIcon.Create(AOwner: TComponent);
begin
...
{$WARNINGS ON}
  ...
  IconData.cbSize := IconData.SizeOf;
  ...
  CycleTimer := TMultimediaTimer.Create(nil);
  CycleTimer.OnTimer := CycleTimerProc;
  ClickTimer := TMultimediaTimer.Create(nil);
  ClickTimer.OnTimer := ClickTimerProc;
  ExitTimer := TMultimediaTimer.Create(nil);
  ExitTimer.Interval := 20;
  ExitTimer.OnTimer := MouseExitTimerProc;
  ...
  begin
    { For MinimizeToTray to work, we need to know when the form is minimized
      ...
      The straight-forward way is to make TZMSTrayIcon trap the
      Application.OnMinimize event. However, if you also make use of this
      event in the application, the OnMinimize code used by TZMSTrayIcon
      is discarded.
...
destructor TZMSTrayIcon.Destroy;
begin
  try
...
function TZMSTrayIcon.LoadDefaultIcon: Boolean;
...
procedure TZMSTrayIcon.Notification(AComponent: TComponent;
...
procedure TZMSTrayIcon.IconChanged(Sender: TObject);
begin
  ModifyIcon;
end;
...
function TZMSTrayIcon.HookAppProc(var Msg: TMessage): Boolean;
var
  Show: Boolean;
  // HideForm: Boolean;
begin
  Result := False;
  // Should always be False unless we don't want the default message handling
...
procedure TZMSTrayIcon.HookForm;
begin
  if (Owner as TWinControl) <> nil then
  begin
    // Hook the parent window
    OldWndProc := Pointer(GetWindowLong((Owner as TWinControl).Handle,
      GWL_WNDPROC));
{$IFDEF DELPHI_6_UP}
...
procedure TZMSTrayIcon.UnhookForm;
begin
...
procedure TZMSTrayIcon.HookFormProc(var Msg: TMessage);
...
procedure TZMSTrayIcon.SetIcon(Value: TIcon);
begin
  FIcon.OnChange := nil;
  // FIcon := Value;
  FIcon.Assign(Value);
...
procedure TZMSTrayIcon.SetIconVisible(Value: Boolean);
begin
  if Value then
    ShowIcon
  else
...
procedure TZMSTrayIcon.SetDesignPreview(Value: Boolean);
begin
...
procedure TZMSTrayIcon.SetCycleIcons(Value: Boolean);
begin
  FCycleIcons := Value;
  if Value then
    SetIconIndex(0);
...
procedure TZMSTrayIcon.SetCycleInterval(Value: Cardinal);
begin
...
procedure TZMSTrayIcon.SetIconList(Value: TCustomImageList);
{$ELSE}

procedure TZMSTrayIcon.SetIconList(Value: TImageList);
{$ENDIF}
begin
  FIconList := Value;
  {
...
procedure TZMSTrayIcon.SetIconIndex(Value: Integer);
begin
...
procedure TZMSTrayIcon.SetHint(Value: THintString);
begin
  FHint := Value;
  ModifyIcon;
end;

procedure TZMSTrayIcon.SetShowHint(Value: Boolean);
begin
  FShowHint := Value;
  ModifyIcon;
end;

procedure TZMSTrayIcon.SetWantEnterExitEvents(Value: Boolean);
begin
  FWantEnterExitEvents := Value;
  ExitTimer.Enabled := Value;
end;

procedure TZMSTrayIcon.SetBehavior(Value: TBehavior);
begin
  FBehavior := Value;
  case FBehavior of
    bhWin95:
      ;
    // IconData.TimeoutOrVersion.uVersion := 0;
    bhWin2000:
      ;
    // IconData.TimeoutOrVersion.uVersion := NOTIFYICON_VERSION;
  end;
  Shell_NotifyIcon(NIM_SETVERSION, @IconData);
end;

function TZMSTrayIcon.InitIcon: Boolean;
...
begin
  ...
  begin
    ...
    begin
      StrLCopy(IconData.szTip, PChar(String(FHint)),
        SizeOf(IconData.szTip) - 1);
      ...
      FillChar(IconData.szTip, SizeOf(IconData.szTip), #0);
...
function TZMSTrayIcon.ShowIcon: Boolean;
...
begin
  ...
  begin
    ...
    else if InitIcon then
      Result := Shell_NotifyIcon(NIM_ADD, @IconData);
  end;
end;

function TZMSTrayIcon.HideIcon: Boolean;
...
function TZMSTrayIcon.ModifyIcon: Boolean;
...
function TZMSTrayIcon.ShowBalloonHint(Title: String; Text: String;
  IconType: TBalloonHintIcon; TimeoutSecs: TBalloonHintTimeOut): Boolean;
// Show balloon hint. Return false if error.
const
  aBalloonIconTypes: array [TBalloonHintIcon] of Byte = (NIIF_NONE, NIIF_INFO,
...
begin
  ...
  begin
    ...
    // StrCopy(szInfo, PAnsiChar(Text), SizeOf(szInfo) - 1);
...
function TZMSTrayIcon.HideBalloonHint: Boolean;
...
function TZMSTrayIcon.BitmapToIcon(const Bitmap: TBitmap; const Icon: TIcon;
  MaskColor: TColor): Boolean;
...
function TZMSTrayIcon.GetClientIconPos(X, Y: Integer): TPoint;
...
function TZMSTrayIcon.GetTooltipHandle: hWnd;
...
function TZMSTrayIcon.GetBalloonHintHandle: hWnd;
...
function TZMSTrayIcon.Refresh: Boolean;
// Refresh the icon
begin
  Result := ModifyIcon;
end;

procedure TZMSTrayIcon.PopupAtCursor;
var
  CursorPos: TPoint;
begin
...
procedure TZMSTrayIcon.Click;
begin
  if Assigned(FOnClick) then
    FOnClick(Self);
end;

procedure TZMSTrayIcon.DblClick;
begin
  if Assigned(FOnDblClick) then
    FOnDblClick(Self);
end;

procedure TZMSTrayIcon.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TZMSTrayIcon.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TZMSTrayIcon.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseMove) then
    FOnMouseMove(Self, Shift, X, Y);
end;

procedure TZMSTrayIcon.MouseEnter;
begin
  if Assigned(FOnMouseEnter) then
    FOnMouseEnter(Self);
end;

procedure TZMSTrayIcon.MouseExit;
begin
  if Assigned(FOnMouseExit) then
    FOnMouseExit(Self);
end;

procedure TZMSTrayIcon.CycleIcon;
var
...
procedure TZMSTrayIcon.DoMinimizeToTray;
begin
...
function TZMSTrayIcon.IsWinNT: Boolean;
var
  ovi: TOSVersionInfo;
  rc: Boolean;
begin
  ...
  begin
    rc := (ovi.dwPlatformId = VER_PLATFORM_WIN32_NT) and
...
procedure TZMSTrayIcon.HideTaskbarIcon;
begin
  if IsWindowVisible(Application.Handle) then
    ShowWindow(Application.Handle, SW_HIDE);
end;

procedure TZMSTrayIcon.ShowTaskbarIcon;
begin
  if not IsWindowVisible(Application.Handle) then
    ShowWindow(Application.Handle, SW_SHOW);
end;

procedure TZMSTrayIcon.ShowMainForm;
begin
  if Owner is TWinControl then // Owner might be of type TService
    if Application.MainForm <> nil then
    begin
...
procedure TZMSTrayIcon.HideMainForm;
begin
  if Owner is TWinControl then // Owner might be of type TService
    if Application.MainForm <> nil then
    begin
