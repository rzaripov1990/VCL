...
begin
  if FUpperCaseKeys then
    Result := WideUpperCase(Copy(FComments[Index], 1, Pos('=', FComments[Index]) - 1))
  else
...
begin
  if Value <> '' then
    if FUpperCaseKeys then
      FComments[Index] := UpperCase(Value) + Copy(FComments[Index], Pos('=', FComments[Index]), MaxInt)
    else
      FComments[Index] := Value + Copy(FComments[Index], Pos('=', FComments[Index]), MaxInt)
  else
    DeleteI(Index);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FComments[FCommentCount - 1] := WideUpperCase(Index) + '=' + AnsiToUtf8(Value);
    end;
  end;
end;
...
begin
  if FUTF8Values then
    Result := UTF8ToWideString(Copy(FComments[Index], 1 + Pos('=', FComments[Index]), MaxInt))
  else
...
begin
  if Value <> '' then
    if FUTF8Values then
      FComments[Index] := Copy(FComments[Index], 1, Pos('=', FComments[Index])) + AnsiToUtf8(Value)
    else
      FComments[Index] := Copy(FComments[Index], 1, Pos('=', FComments[Index])) + Value
  else
    DeleteI(Index);
end;
