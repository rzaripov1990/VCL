...
const
  OFR_COMPRESSION: array [0 .. 9] of String = ('fast', 'normal', 'high', 'extra', 'best', 'ultra', 'insane', 'highnew',
    'extranew', 'bestnew');
...
begin
  Result := (FHeader.ID = 'OFR ') and (FHeader.SampleRate > 0) and (FHeader.SampleType in [0 .. 10]) and
    (FHeader.ChannelMode in [0 .. 1]) and (FHeader.CompressionID shr 3 in [0 .. 9]);
end;
...
begin
  ...
  if FGetValid then
    Result := FFileLength / (FGetSamples * ((FHeader.ChannelMode + 1) * Abs(FGetBits) / 8) + 44) * 100
  else
    Result := 0;
end;
