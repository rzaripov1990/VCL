...
unit SpeexTagLibrary;

interface

uses
  Classes, SysUtils, VorbisCommentTagLibrary;
...
type
  ...
  public
    ...
    property VorbisComment: TVorbisComment read vComments write vComments;

    property isSpeexTag: boolean read isSpeex;
...
const
  ...
  VORBIS_PARAMETERS_ID: AnsiString = #1 + 'vorbis';
  SPEEX_PARAMETERS_ID: AnsiString = 'Speex   ';

  { Vorbis tag frame ID }
  VORBIS_TAG_ID: AnsiString = #3 + 'vorbis';
...
type
  ...
  OggHeader = packed record
    { 0-3 }ID: array[1..4] of AnsiChar; { Always "OggS" }
  ...
  VorbisHeader = packed record
    { 7 }ID: array[1..7] of AnsiChar; { Always #1 + "vorbis" }
...
begin
  ...
  begin
    ...
    for Iterator := SizeOf(Data) - 10 downto 0 do
      if Data[Iterator] + Data[Iterator + 1] + Data[Iterator + 2] + Data[Iterator + 3] = OGG_PAGE_ID then
      begin
...
var
  ...
  Last: boolean;
  Size: Integer;
  VorbisTagID: array[1..7] of AnsiChar; { Always #3 + "vorbis" }
...
begin
  ...
  try
    ...
    { First Page }
    SourceFile.Read(Info.FPage, OggHeaderSize_27 { SizeOf(Info.FPage) });
    ...
    if AnsiStrLComp(PAnsiChar(VORBIS_PARAMETERS_ID), Data, Length(VORBIS_PARAMETERS_ID)) = 0 then
    begin
      Info.isOggVorbis := True;
      Move(Data^, Info.VorbisParameters, SizeOf(Info.VorbisParameters));
    end
    else if AnsiStrLComp(PAnsiChar(SPEEX_PARAMETERS_ID), Data, Length(SPEEX_PARAMETERS_ID)) = 0 then
    begin
      Info.isSpeex := True;
    ...
    { Second Page }
    SourceFile.Read(Info.SPage, OggHeaderSize_27 { SizeOf(Info.SPage) });
    ...
    CommentsStream.CopyFrom(SourceFile, Size);
    ...
    begin
      SourceFile.Read(Info.LPage, OggHeaderSize_27 { SizeOf(Info.SPage) });
...
begin
  ...
  if Info.isOggVorbis then
    Result.Write(PAnsiChar(VORBIS_TAG_ID)^, Length(VORBIS_TAG_ID));
...
begin
  ...
  Value := Value + NewTagSize - (Info.TagEndPos - Info.SPagePos - Info.SPage.Segments - OggHeaderSize_27);
  ...
  if Position < Info.SPage.Segments then
    for Index := Position + 1 to Info.SPage.Segments do
      Buffer[Index - Position + (Value div $FF) + 1] := Info.SPage.LacingValues[Index];
...
begin
  ...
  Result := (FChannelModeID in [VORBIS_CM_MONO, VORBIS_CM_STEREO]) and (SampleRate > 0) and (Duration > 0.1)
    and (BitRate > 0);
end;
