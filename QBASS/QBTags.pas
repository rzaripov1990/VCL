...
const
  ...
  { Cover Type }
  QBASS_CoverType: array [0 .. 20] of AnsiString = ('Other',
    '32x32 pixels '#39'file icon'#39' (PNG only)', 'Other file icon',
...
type
  ...
  QBASSPictureInfo = record
    ...
    mime: AnsiString; // mime type
    desc: AnsiString; // description
  end;

  // Read tags from bass and bass add-ons
function QBass_ReadTags(const Channel: DWORD; var Item: QBASSTagItem): boolean;
function QBass_ReadNetTags(const Channel: DWORD;
  var Item: QBASSTagItem): boolean;
function QBass_ReadCover(const Channel: DWORD; var Info: QBASSPictureInfo;
  var Image: TBytesStream): boolean;
...
const
  ...
  GENRE_TABLE: array [0 .. MAX_MUSIC_GENRES - 1] of AnsiString = ('Blues',
  ...
  // id3v2.2
  ID3V2_2: array [1 .. ID3V2_FRAME_COUNT] of AnsiString = ('TT2', 'TP1', 'TCO',
    'TOA', 'TT1', 'TYE', 'TOR', 'COM', 'TAL', 'TOA', '');

  // id3v2.3, id3v2.4
  ID3V2_34: array [1 .. ID3V2_FRAME_COUNT] of AnsiString = ('TIT2', 'TPE1',
    'TCON', 'TOPE', 'TIT1', 'TYER', 'TDRC', 'COMM', 'TALB', 'TOAL', 'APIC');

  // table for OGG, APE, OFR, MPC, AAC, FLAC
  GEN_NAME: array [0 .. GEN_FRAME_COUNT] of AnsiString = ('ARTIST=', 'TITLE=',
    'ALBUM=', 'GENRE=', 'DATE=', 'COMMENT=', 'LYRICS=', 'CUESHEET=');

  // wma version <= 2.3
  WMA_23: array [0 .. WMA_FRAME_COUNT] of AnsiString = ('Author:', 'Title:',
    'WM/AlbumTitle:', 'WM/Genre:', 'WM/Year:', 'Description:', 'WM/Author:',
    'WM/Title:', 'WM/LYRICS:');

  // wma version >= 2.4
  WMA_24: array [0 .. WMA_FRAME_COUNT] of AnsiString = ('Author=', 'Title=',
    'WM/AlbumTitle=', 'WM/Genre=', 'WM/Year=', 'Description=', 'WM/Author=',
    'WM/Title=', 'WM/LYRICS=');
...
begin
  Result := (((Byte(aChar) <= $7F) or (($C2 <= Byte(aChar)) and
    (Byte(aChar) <= $FD))) or ($80 <= Byte(aChar)) and (Byte(aChar) <= $BF));
end;
...
begin
  ...
  begin
    ...
    Result := Trim({$IFDEF UNICODE}UTF8ToString(RawByteString(Source))
      ){$ELSE}Utf8ToAnsi(Source)){$ENDIF};
end;
...
function ParseStr(var Str: AnsiString; const sBegin, sEnd: AnsiString;
  FindDelete: boolean = false): AnsiString;
// парсер строк
const
  endStr = '*^*';
var
...
begin
  try
    Result := (Ord((data)^) shl 24) + (Ord((data + 1)^) shl 16) +
      (Ord((data + 2)^) shl 8) + (Ord((data + 3)^));
  except
    Result := 0;
  end;
end;
...
function ID3FindFrame(data: PAnsiChar; FrameName: String; var tag: TagID3v2)
  : PAnsiChar;
var
  DataTemp: PAnsiChar;
...
function frame_reader(data: PAnsiChar; fmt: String; var tag: TagID3v2)
  : AnsiString;
var
  temp: PAnsiChar;
  siz: Integer;
...
function genre_reader(data: PAnsiChar; fmt: String; var tag: TagID3v2)
  : AnsiString;
begin
...
function comment_reader(data: PAnsiChar; fmt: String; var tag: TagID3v2)
  : AnsiString;
var
  temp: PAnsiChar;
  siz, I: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        with TFileStream.Create(Item.FileName, fmOpenRead or
          fmShareDenyWrite) do
        begin
          tag.FileSize := Size;
          Free;
      ...
      begin
        Result := true;
        Item.Artist := IFV(frame_reader(data, ID3V2_34[2], tag),
          frame_reader(data, ID3V2_34[4], tag));
        Item.Title := IFV(frame_reader(data, ID3V2_34[1], tag),
          frame_reader(data, ID3V2_34[5], tag));
        Item.Album := IFV(frame_reader(data, ID3V2_34[9], tag),
          frame_reader(data, ID3V2_34[10], tag));
...
begin
  ...
  try
    if data <> nil then
      Result := (data)^ + (data + 1)^ + (data + 2)^ + (data + 3)
        ^ = Utf8ToAnsi(ID3V2_34[11]);
  except
    Result := false;
  end;
end;
...
function DetectID3V2Cover(struct: Pointer; var Info: QBASSPictureInfo;
  var Image: TBytesStream): boolean;
var
  id3: TagID3v2;
  temp: PAnsiChar;
...
function DetectFLACCover(struct: PAnsiChar; var Info: QBASSPictureInfo;
  var Image: TBytesStream): boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      Item.Title := t;
      Item.Album := IFV(tag_find(struct, WMA_23[2]),
        tag_find(struct, WMA_24[2]));
      Item.Genre := IFV(tag_find(struct, WMA_23[3]),
        tag_find(struct, WMA_24[3]));
      Item.Year := IFV(tag_find(struct, WMA_23[4]),
        tag_find(struct, WMA_24[4]));
      Item.Comment := IFV(tag_find(struct, WMA_23[5]),
        tag_find(struct, WMA_24[5]));
      Item.Lyrics := IFV(tag_find(struct, WMA_23[8]),
        tag_find(struct, WMA_24[8]));
    end;
  except
    Result := false;
  end;
  ...
  begin
    ...
    begin
      try
        ...
        else if (copy(s, 1, 17) = 'icy-samplerate=') then
          Item.Lyrics := copy(s, 18, MaxInt)
...
begin
  ...
  begin
    ...
    begin
      if not DetectID3v2(BASS_ChannelGetTags(Channel, BASS_TAG_ID3V2), Item)
      then
    ...
    begin
      if not DetectGeneral(BASS_ChannelGetTags(Channel, BASS_TAG_OGG), Item)
      then
    ...
    begin
      if not DetectID3v2(BASS_ChannelGetTags(Channel, BASS_TAG_ID3V2), Item)
      then
        if not DetectID3v1(BASS_ChannelGetTags(Channel, BASS_TAG_ID3), Item)
        then
          if not DetectGeneral(BASS_ChannelGetTags(Channel, BASS_TAG_APE), Item)
          then
...
begin
  ...
  if Channel > 0 then
  begin
    Result := DetectID3V2Cover(Pointer(BASS_ChannelGetTags(Channel,
      BASS_TAG_ID3V2)), Info, Image);
    if not Result then
      Result := DetectFLACCover(BASS_ChannelGetTags(Channel,
        BASS_TAG_FLAC_PICTURE), Info, Image);
  end;
end;
