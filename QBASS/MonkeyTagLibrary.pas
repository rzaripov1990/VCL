...
const
  ...
  MONKEY_COMPRESSION: array [0 .. 6] of string = ('Unknown', 'Fast', 'Normal', 'High', 'Extra High', 'Insane', 'BrainDead');
...
begin
  ...
  FResetData;
  // load tags first
  FID3v2.LoadFromFile(FileName);
  FID3v1.LoadFromFile(FileName);
  FAPEtag.LoadFromFile(FileName);
  // calculate total tag size
  TagSize := 0;
  if FID3v1.Loaded then
    inc(TagSize, 128);
  ...
  try
    try
      ...
      if (f.Read(APE, sizeof(APE)) = sizeof(APE)) and (StrLComp(@APE.cID[0], 'MAC ', 4) = 0) then
      begin
        ...
        begin
          ...
          begin
            ...
            if (longword(f.Read(APE_NEW, APE_DESC.nHeaderBytes)) = APE_DESC.nHeaderBytes) then
            begin
              ...
              begin
                FTotalSamples := int64(APE_NEW.nBlocksPerFrame) * int64(APE_NEW.nTotalFrames - 1) +
        ...
        begin
          ...
          begin
            ...
            else if (FVersion >= 3900) or ((FVersion >= 3800) and (APE_OLD.nCompressionLevel = MONKEY_COMPRESSION_EXTRA_HIGH))
            then
            ...
            begin
              FTotalSamples := int64(APE_OLD.nTotalFrames - 1) * int64(BlocksPerFrame) + int64(APE_OLD.nFinalFrameBlocks);
            end;
            LoadSuccess := TRUE;
          end;
        end;
        ...
        begin
          ...
          if FSampleRate > 0 then
            FDuration := FTotalSamples / FSampleRate;
          // average bitrate
          if FDuration > 0 then
            FBitrate := (FFileSize - int64(TagSize)) * 8.0 / (FDuration / 1000.0);
