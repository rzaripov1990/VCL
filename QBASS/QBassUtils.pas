...
uses
  WinApi.Windows, VCl.Graphics, System.SysUtils, BASS, QBCommon, SLCanvas32,
  QBTags, QBDSP, QBVis, Dialogs;
...
const
  ...
  QBASS_GET_CHANNEL_MODE = 10; // stereo/mono?
  QBASS_GET_BITS_PER_SAMPLE = 11; // bits per sample
  ...
  QBASS_DSP_ROTATE = 6;
  QBASS_DSP_ECHO = 7;
  QBASS_DSP_DOUBLEVOLUME = 8;

  // equalizer mode
  QBASS_EQ_MODE_10 = 0;
  QBASS_EQ_MODE_18 = 1;

  QBASS_DEFAULT_FORMATS =
    '*.mp3;*.mp2;*.mp1;*.oga;*.ogg;*.wav;*.aif;*.aiff;*.mo3;*.it;*.xm;*.s3m;*.mtm;*.mod;*.umx;';

  QBASSErrorCodes: array[0..46] of string = ('All is OK', 'Memory error', 'Can''t open the file',
    'Can''t find a free sound driver', 'The sample buffer was lost', 'Invalid handle',
    ...
    'Unknown error',
    'Not paused', 'Unknown error', 'Can''t get a free channel', 'An illegal type was specified',
    'An illegal parameter was specified', 'No 3D support', 'No EAX support', 'Illegal device number',
    'Not playing',
    'Illegal sample rate', 'Unknown error', 'The stream is not a file stream', 'Unknown error',
    'No hardware voices available',
    'Unknown error', 'The MOD music has no sequence data', 'No internet connection could be opened',
    'Couldn''t create the file',
    'Effects are not enabled', 'The channel is playing', 'Unknown error', 'Requested data is not available',
    'The channel is a "decoding channel"', 'A sufficient DirectX version is not installed',
    'Connection timedout',
    'Unsupported file format', 'Unavailable speaker', 'Invalid BASS version (used by add-ons)',
    'Codec is not available/supported', 'The channel/file has ended', 'The device is busy');
...
function QBASS_SetBuffer(const Value: Cardinal): Single;
function QBASS_GetBuffer: Cardinal;
...
function QBass_EQGetPreamp: Single;
procedure QBass_EQSetPreamp(const Value: Single);
...
const
  ...
  QBands: TEQBandCenter = (80, 170, 310, 600, 1000, 3000, 6000, 10000, 12000, 14000);
  // eq band center frequence
  QBands18: TEQBandCenter18 = (80, 120, 200, 350, 550, 750, 1000, 1800, 2500, 3800, 5000, 6200, 7500, 9000,
    11000, 12000,
    13000, 14000);
...
var
  ...
  // dsp proc
  amp: AutoVolume;
  LC: DWORD = 0; // left channel
  RC: DWORD = 0; // right channel
  VF: DWORD = 0; // voice filter
  CS: DWORD = 0; // channel swap
  AV: DWORD = 0; // auto volume
  FE: DWORD = 0; // fading effect
  FL: DWORD = 0; // flange
  EC: DWORD = 0; // echo
  DV: DWORD = 0; // double volume
  RT: DWORD = 0; // rotate
...
begin
  ...
  case Flag of
    ...
    QBASS_GET_CHANNEL_MODE:
      begin
        BASS_ChannelGetInfo(Channel, bch);
        Result := bch.chans;
      end;
    QBASS_GET_BITS_PER_SAMPLE:
      begin
        BASS_ChannelGetInfo(Channel, bch);
        Result := bch.origres;
      end;
...
begin
  Result := false;
  if (Copy(LowerCase(s), 1, 7) = 'http://') or (Copy(LowerCase(s), 1, 6) = 'ftp://') or (Copy(LowerCase(s),
    1, 6) = 'mms://') then
    Result := true;
end;
// ******************************************************************************
...
begin
  if FilesMask = '' then
  begin
    FilesMask := 'All supported formats|' + QBASS_DEFAULT_FORMATS + FilesMask + '|' +
...
begin
  ...
  begin
    ...
    repeat
      ...
      begin
        pluginfo := nil;
        bassplug := BASS_PluginLoad(Pointer(Path + SearchRec.name), Flags);
        if bassplug > 0 then
        begin
          ...
          begin
            ...
            if not (Pos(ext, FilesMask) > 0) then
              FilesMask := FilesMask + ext;
          end;
...
begin
  Result := 0.0;
  if (not QBass_IsInit) then
    exit;

  v := Value * 10;
  BASS_SetConfig(BASS_CONFIG_BUFFER, v);
  Result := Value / 100;
end;
...
begin
  ...
  Result := BASS_GetConfig(BASS_CONFIG_BUFFER);
end;
// ******************************************************************************
...
procedure DSPProc_VoiceOff(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: Pointer);
  stdcall;
var
  i: DWORD;
  dmch: Single;
...
procedure DSPProc_LeftChannel(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: Pointer);
  stdcall;
var
  i: DWORD;
  lch, rch: PSingle;
...
procedure DSPProc_RightChannel(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: Pointer);
  stdcall;
var
  i: DWORD;
  lch, rch: PSingle;
...
procedure DSPProc_SwapChannel(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: Pointer);
  stdcall;
var
  lch, rch: PSingle;
  Temp: PSingle;
...
procedure DSPProc_Rotate(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: DWORD); stdcall;
var
  a: DWORD;
  d: PSingle;
begin
...
procedure DSPProc_Echo(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: DWORD); stdcall;
var
  a: DWORD;
  d: PSingle;
  l, r: Single;
...
procedure DSPProc_Flange(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: DWORD); stdcall;
var
  a: DWORD;
  d: PSingle;
  f, s: Single;
...
procedure DSPProc_DoubleVolume(DSPHandle: HDSP; Channel: DWORD; buffer: PSample; length: DWORD; var User:
  DWORD); stdcall; export;
var
  i: Cardinal;
begin
  try
...
begin
  //
  Result := Round(SampleValue * amp.gain);
  // amplify sample. "усиленое" значение
...
procedure DSPProc_AutoAmp32(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: Pointer);
  stdcall;
var
  c: DWORD;
  d: integer;
begin
  ...
  begin
    // ---------Сингловский сэмпл----------//
    d := Trunc(Single(Pointer(Cardinal(buffer) + c * 4)^) * 32768);
    // Получаем сэмпл и преобразовываем в целое
    //
    if d > 32767 then
      d := 32767
...
begin
  ...
  case fx of
    ...
    QBASS_DSP_FLANGE:
      begin
        if FL > 0 then
          Result := BASS_ChannelRemoveDSP(Channel, FL);
        FL := 0;
        ...
        begin
          ...
          Flas := FLABUFLEN / 2;
          Flasinc := 0.002;
          FL := BASS_ChannelSetDSP(Channel, @DSPProc_Flange, nil, 2);
          Result := FL > 0;
        end;
      end;
    ...
    QBASS_DSP_ROTATE:
      begin
        if RT > 0 then
          Result := BASS_ChannelRemoveDSP(Channel, RT);
        RT := 0;

        if Activate then
        begin
          RotPos := 0.7853981;
          RT := BASS_ChannelSetDSP(Channel, @DSPProc_Rotate, nil, 2);
          Result := RT > 0;
        end;
      end;
    ...
    QBASS_DSP_ECHO:
      begin
        if EC > 0 then
          Result := BASS_ChannelRemoveDSP(Channel, EC);
        EC := 0;
        ...
        begin
          ...
          EchPos := 0;
          EC := BASS_ChannelSetDSP(Channel, @DSPProc_Echo, nil, 2);
          Result := EC > 0;
        end;
      end;

    QBASS_DSP_DOUBLEVOLUME:
      begin
        if DV > 0 then
          Result := BASS_ChannelRemoveDSP(Channel, DV);
        DV := 0;

        if Activate then
        begin
          DV := BASS_ChannelSetDSP(Channel, @DSPProc_DoubleVolume, nil, 2);
          Result := DV > 0;
        end;
      end;
  else
...
procedure DSPProc_PreAmp(Handle: HDSP; Channel: DWORD; buffer: Pointer; length: DWORD; User: Pointer);
  stdcall;
var
  i: DWORD;
  lch, rch: PSingle;
